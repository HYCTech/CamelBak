  ...
  components:{
   ...
   data() {
    ...
    methods: {
      ...
      //获取分页数据
      getInfo() {
        api.getOrder(this.pageData.page,this.pageData.pageSize,{"order_state":"finished","order_type":"personal"}).then(res=>{
    ...
    delItem() {
      ...
      search(searchObject) {    
         if(searchObject.value&&!!searchObject.select){
             api.getOrder(this.pageData.page,this.pageData.pageSize,{[searchObject.select]:searchObject.value,"order_state":"finished","order_type":"personal"})
                .then(res=>{
                  this.tableData=res.data
                  this.pageData.total=res.total
                })
