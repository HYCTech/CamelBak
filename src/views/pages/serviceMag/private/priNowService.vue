   ...
   <SearchBox :onsearch="search" :labelItem="searchLimit"></SearchBox>
    ...
    <div>
     <el-table highlight-current-row :data="tableData" border style="width: 100%" ref="table" :default-sort="{prop: 'date', order: 'descending'}"
        ...
        <el-table-column prop="repair_place" label="维修位置" width="100">
        </el-table-column>
...
<script>
  ...
  export default {
   ...
   methods: {
      ...
      //获取分页数据
      getInfo() {
        api.getOrder(this.pageData.page,this.pageData.pageSize,{"order_state":"repairing","order_type":"personal"}).then(res=>{
    ...
    delItem() {
      ...
      search(searchObject) {     
         if(searchObject.value&&!!searchObject.select){
             api.getOrder(this.pageData.page,this.pageData.pageSize,{[searchObject.select]:searchObject.value,"order_state":"repairing","order_type":"personal"})
                .then(res=>{
                  this.tableData=res.data
                  this.pageData.total=res.total
                })
