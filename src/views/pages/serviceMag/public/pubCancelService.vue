     ...
     <el-table highlight-current-row :data="tableData" border style="width: 100%" ref="table" :default-sort="{prop: 'date', order: 'descending'}"
        ...
        <el-table-column prop="picture" label="图片">
          ...
          <el-popover
            ...
            title="大图"
            trigger="hover">
             <img :src="item.minFilename" title="点击查看大图" style="width:60px;height:100px;" @click="showBigImg(item.filename)" class="img-item" alt="" v-for="(item,index) in scope.row.picture">
          </el-popover>
...
<script>
  ...
  export default {
   ...
   methods: {
      ...
      //获取分页数据
      getInfo() {
        api.getOrder(this.pageData.page,this.pageData.pageSize,{"order_state":"cancel","order_type":"public"}).then(res=>{
    ...
    delItem() {
      ...
      search(searchObject) {
        ...
        if(searchObject.value&&!!searchObject.select){
            api.getOrder(this.pageData.page,this.pageData.pageSize,{[searchObject.select]:searchObject.value,"order_state":"cancel","order_type":"public"})
               .then(res=>{
                  this.tableData=res.data
                  this.pageData.total=res.total
                    })
